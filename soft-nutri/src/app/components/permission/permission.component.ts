...
export class PermissionComponent implements OnInit {
  ...
  save() {
    if (this.userControl.value != '') {
      ...
      this.snackBar.open(
        this.translate.instant('PERMISSION.ERROR_USER'),
        'Error',
        {
          horizontalPosition: 'center',
          verticalPosition: 'top',
  ...
  private listPermissionUser() {
    this.permissionService
      .get(this.userSelected.idPerson)
      .then((data) => {
        debugger;
  ...
  updatePermission(option: Permission) {
    ...
    for (let i = 0; i < option.paper.length; i++) {
      ...
      if (option.paper[i].post != -1) {
        option.checked
          ? (option.paper[i].post = 1)
          : (option.paper[i].post = 0);
      }
      ...
      if (option.paper[i].delete != -1) {
        option.checked
          ? (option.paper[i].delete = 1)
          : (option.paper[i].delete = 0);
      }
    }
