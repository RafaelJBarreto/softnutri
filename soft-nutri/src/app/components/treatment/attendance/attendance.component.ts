...
import { FoodAttendanceComponent } from './food-attendance/food-attendance.component';
...
export class AttendanceComponent implements OnInit {
  ...
  private listTable() {
    this.tableService.listAll().then(
      (data) => {
        this.tables = data;
        this.filteredOptionsTable = this.tableControl.valueChanges.pipe(
          startWith(''),
      ...
      if (idSnackMenu === this.listSnackMenu[sm].idSnackMenu) {
        for (let s = 0; s < this.listSnackMenu[sm].listSnackMenu.length; s++) {
          if (idSnack === this.listSnackMenu[sm].listSnackMenu[s].idSnack) {
            this.listSnackMenu[sm].listSnackMenu[s].selected = !this.listSnackMenu[sm].listSnackMenu[s].selected;
            if (this.listSnackMenu[sm].listSnackMenu[s].selected && this.listSnackMenu[sm].listBreakFast.length === 0) {
              this.listSnackMenu[sm].listBreakFast.push(
                new FoodMenu(this.listSnackMenu[sm].listBreakFast.length + 1, idSnackMenu, 0, 0, 0, 0, 0, 0)
              );
            }
          } else {
  ...
  private setValueObject() {
    const sm: SnackMenu = {
      ...
      listSnackMenu: structuredClone(this.snacks),
      listBreakFast: structuredClone(this.listBreakFast)
    };
